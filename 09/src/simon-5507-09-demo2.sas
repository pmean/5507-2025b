...
ods pdf 
    file= "&path/results/simon-5507-08-demo2.pdf";
...
* Comments on the code: The intnx function will 
    ...
    weeks, months, or years from a date. 
    
For this example, the statement, weekend_a = 
    opening_date-2, would also work. But try 
    to get in the habit of using the intnx 
    function because it more gracefully 
    handles month and year calculations 
    (with the complexities with different 
    days in each month and with leap year
    considerations).;
...
The discrete approach would count 12 months between
    Sorceror's Stone and Chamber of Secrets because
    you flipped the calendar twelve times to get 
    from 11-18-2001 to 11-17-2002. The continuous 
    approach would not count the last month until
    you had gone at least 18 days into the last 
    month. In most research settings, you will 
    probably want the continuous approach.

Both the discrete and continuous approaches produce
    a whole number. To get a fractional number of 
    months, calculate the number of days between 
    the dates and divide by 365.25 to get a 
    fractional number of years and then multiply by
    12 to get a fractional number of months.

An odd quirk of SAS is that proc print will often 
    display a date value as the number of days 
    since January 1, 1960. Use the format
